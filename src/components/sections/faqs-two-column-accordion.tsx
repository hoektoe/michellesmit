  ...
  return (
    <div id={id} {...props}>
      <button
        ...
        className="flex w-full items-start justify-between gap-6 py-4 text-left text-base/7 text-mist-950"
      >
        {question}
      ...
      <ElDisclosure
        id={`${id}-answer`}
        hidden
        className="-mt-2 flex flex-col gap-2 pr-12 pb-4 text-sm/7 text-mist-700"
      >
        {answer}
      </ElDisclosure>
    </div>
  ...
  return (
    <section className={clsx('py-16', className)} {...props}>
      <Container className="grid grid-cols-1 gap-x-2 gap-y-8 lg:grid-cols-2">
        ...
        <div className="divide-y divide-mist-950/10 border-y border-mist-950/10">
          {children}
        </div>
      </Container>
    </section>
